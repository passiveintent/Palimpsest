...
func encodeBlob(t *testing.T, entries []wire.SnapshotEntry, gzipped bool) []byte {
	t.Helper()
	blob, err := wire.EncodeSnapshot(entries, gzipped)
	if err != nil {
...
func TestSelectAndPersist_FiltersToFlaggedIDs(t *testing.T) {
	...
	blob := encodeBlob(t, entries, true)

	now := time.Unix(100, 0)
	snaps, err := s.SelectAndPersist([]uint64{1, 3}, blob, now)
	if err != nil {
...
func TestSelectAndPersist_UncompressedBlob(t *testing.T) {
	...
	blob := encodeBlob(t, entries, false)

	snaps, err := s.SelectAndPersist([]uint64{42}, blob, time.Unix(1, 0))
	if err != nil {
		t.Fatalf("SelectAndPersist: %v", err)
	}
	if len(snaps) != 1 {
...
func TestSnapshotStore_TTLEviction(t *testing.T) {
	...
	blob := encodeBlob(t, []wire.SnapshotEntry{{ID: 1, TSMs: 0, Value: 1}}, true)
	start := time.Unix(1000, 0)
	if _, err := s.SelectAndPersist([]uint64{1}, blob, start); err != nil {
		t.Fatalf("SelectAndPersist: %v", err)
	}
...
func TestSnapshotStore_CapacityEviction(t *testing.T) {
	...
	for id := uint64(1); id <= 3; id++ {
		blob := encodeBlob(t, []wire.SnapshotEntry{{ID: id, TSMs: 0, Value: 1}}, true)
		if _, err := s.SelectAndPersist([]uint64{id}, blob, now); err != nil {
...
func TestSelectAndPersist_NoMatchesReturnsNil(t *testing.T) {
	...
	blob := encodeBlob(t, []wire.SnapshotEntry{{ID: 1, TSMs: 0, Value: 1}}, true)
	snaps, err := s.SelectAndPersist([]uint64{999}, blob, time.Unix(1, 0))
	if err != nil {
		t.Fatalf("SelectAndPersist: %v", err)
	}
	if len(snaps) != 0 {
